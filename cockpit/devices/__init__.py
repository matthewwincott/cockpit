...
* :mod:`cockpit.devices.microscopeDevice` module with all Python
  Microscope adaptors with excluding adaptors for cameras.
...
The long term plan is to have all device control handled by Python
Microscope.  When that happens, only the Python Microscope adaptors
should be kept and they will no longer need to be referred as adaptors
since they will be the only ones.  At that time, the separation
between Cockpit devices and device handlers can probably be removed.
We might also find that highly specialised devices will continue to
require a specialised Cockpit device class, such as the Aurox Clarity,
although they may be better distributed as their own Python package.
