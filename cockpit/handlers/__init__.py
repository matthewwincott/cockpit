...
`PositionerHandler` has `moveRelative()` and `moveAbsolute()`.  These
methods in turn invoke callback functions specified by the Cockpit
device itself, which then presumably talks to the hardware.  For
information on the required and optional callbacks for each handler,
check their source code.

In short, Cockpit talks to the `DeviceHandler`, the `DeviceHandler`
talks a `Device`, and a `Device` talks to the hardware.
...
hardware is handled by Microscope we should be able to remove the
devices and handlers abstraction layers.
